...
def test_scrape_news_html_content(
    mock_async_html_loader, html_content_document, text_document
):
...
def test_process_raw_html_doc(
    text_documents, chunk_size, chunk_overlap, expected_chunks
):
    html2text_transfomer = Html2TextTransformer()
    text_splitter = RecursiveCharacterTextSplitter(
        chunk_size=chunk_size, chunk_overlap=chunk_overlap
    )
    chunks = process_raw_html_doc(text_documents, html2text_transfomer, text_splitter)
